...
>
  ...
  <div class="flex items-center">
    ...
    <div class="ml-auto hidden gap-2 lg:flex">
      ...
      @php
          $refundEligible = app(\Remix\RefundRequest\Services\RefundEligibilityService::class)->canRequestRefund($order);
          $existingRefund = $order->remixRefundRequests()->latest()->first();
      @endphp
      @if($refundEligible)
          <x-shop::ui.button
              variant="outline"
              color="danger"
              size="sm"
              href="{{ route('remix.refund_request.customer.create', $order->id) }}"
          >
              Ajukan Refund
          </x-shop::ui.button>
      @elseif($existingRefund)
          <x-shop::ui.button
              ...
              color="info"
              size="sm"
              href="{{ route('remix.refund_request.customer.show', $existingRefund->id) }}"
          >
              Lihat Status Refund
          </x-shop::ui.button>
      @endif
